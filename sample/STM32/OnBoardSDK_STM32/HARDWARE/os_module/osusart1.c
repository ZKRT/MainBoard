...
extern OS_EVENT *Plc_com_mbox;				//mail box
//OS_EVENT *Plc_com_mbox;				//mail box
...
volatile  u8 							 _u1com1_state;				//uart state in 3 seconds
volatile  u8 							 _u1com1_state_tmrcnt;	//timer count
...
//	_serial_dat_ptr usart1_m;
//	
//	ReadFlash_Status = Read_Flash_Onebyte(GetFlash_dat);
//	while(1 != ReadFlash_Status);
//	usart1_m = GetFlash_dat
//	
...
static  void  _hwuart1_rxIntrSer(void)
{
  if(_u1RcvStates == URX_BUF_IDLE || _u1RcvStates == URX_BUF_BUSY) //and (_u1RxEn == ENABLE)) 
    ...
    _pu1RxPtr->pscommRcvContent[_u1rxCnt] = USART_ReceiveData(EVAL_COM1);
		_u1rxCnt++;
...
static  void  _hwuart1_txIntrSer(void)
{
  if(_pu1TxPtr != NULL)
  {
    ...
    {
			/* Write one byte to the transmit data register */
			USART_SendData(EVAL_COM1, _pu1TxPtr->pscommTxContent[_u1txCnt]);
			_u1txCnt++;
...
//    }     
  ...
  if(_u1RcvStates == URX_BUF_COMPL)
  {
    ...
    _pu1RxPtr->scommRcvLen = _u1rxCnt;
		...
		OSMboxPost(Plc_com_mbox,(void *)_pu1RxPtr); 	//ucos application: send msg to mail box use
//    _u1TxEn = ENABLE;
//    if(_u1Mode == SCOMM_MODE_MASTER)
//    {
//      _u1FrameTmr = 0;
...
//			USART_SendData(EVAL_COM1, _pu1TxPtr->pscommTxContent[0]);//[_u1txCnt-1]);
      return(SCOMM_RET_OK);
    }
    else
    {
...
{
	return ((USART_GetITStatus(EVAL_COM1, USART_IT_TXE) == SET) ||(_u1Txing == TRUE));
}
void os_usart1(void)
{
	Usart1TaskInit();
...
void os_reset_rcvstate_u1(void)			//mail box use
{
	_pu1RxPtr->scommRcvLen = 0;   //reset value
	_u1RcvStates = URX_BUF_IDLE; 
}	
u8 os_get_uart_chls_u1(void)
{
