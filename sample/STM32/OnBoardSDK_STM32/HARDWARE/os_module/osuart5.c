...
extern OS_EVENT *Rs232_mbox;		//mail box
//OS_EVENT *Rs232_mbox;		//mail box
...
volatile  u8 							 _u1com5_state;				//uart state in 3 seconds
volatile  u8 							 _u1com5_state_tmrcnt;	//timer count
...
static void Usart1Config(void)
{
	...
	USART_InitStructure.USART_BaudRate = baudrate_map[dev_info.rs232.baudrate];
  USART_InitStructure.USART_WordLength = databit_map[dev_info.rs232.databit];
  USART_InitStructure.USART_StopBits = stopbit_map[dev_info.rs232.stopbit];
  USART_InitStructure.USART_Parity = parity_map[dev_info.rs232.parity];
  USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
  USART_InitStructure.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;

  STM_EVAL_COMInit(COM5, &USART_InitStructure);																									//charge com need to change
	
  /* Enable the EVAL_COM5 Transmoit interrupt: this interrupt is generated when the 
     EVAL_COM5 transmit data register is empty */  
//  USART_ITConfig(EVAL_COM2, USART_IT_TXE, ENABLE);
	USART_ITConfig(EVAL_COM5, USART_IT_TXE, DISABLE); 												//charge com need to change

  /* Enable the EVAL_COM5 Receive interrupt: this interrupt is generated  
     when the EVAL_COM5 receive data register is not empty */
//  USART_ITConfig(EVAL_COM5, USART_IT_RXNE, ENABLE);	//charge com need to change
	if(dev_info.rs232.enalbed & 0x01)	
		USART_ITConfig(EVAL_COM5, USART_IT_RXNE, ENABLE); 	//modify by yanly
	else
		USART_ITConfig(EVAL_COM5, USART_IT_RXNE, DISABLE); 
}
void usart5_config_chg(void)
{
	USART_InitTypeDef USART_InitStructure;
  ...
  USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
  USART_InitStructure.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;
  STM_EVAL_COMInit(COM5, &USART_InitStructure);							
	if(dev_info.rs232.enalbed & 0x01)	
		USART_ITConfig(EVAL_COM5, USART_IT_RXNE, ENABLE);
	else
		USART_ITConfig(EVAL_COM5, USART_IT_RXNE, DISABLE); 
}
/******interupt fun handler***********/
void _UART5_IRQHandler(void)
{
  if (USART_GetITStatus(EVAL_COM5, USART_IT_RXNE) != RESET)	//charge com need to change
  {
    /* received data */
  ...
  {   
    ...
    //USART_SendBufferData();
		...
		//add by yanly160705
	_u1com5_state = UART_CHANNEL_BUSY;
  _u1com5_state_tmrcnt = UART_CHANNEL_BUSY_TIMEROUT;
}
static void Usart1RstVariable(void)
{
...
static  void  _hwuart1_rxIntrSer(void)
{
  if(_u1RcvStates == URX_BUF_IDLE || _u1RcvStates == URX_BUF_BUSY) //and (_u1RxEn == ENABLE)) 
    ...
    _pu1RxPtr->pscommRcvContent[_u1rxCnt] = USART_ReceiveData(EVAL_COM5);
		_u1rxCnt++;
...
static  void  _hwuart1_txIntrSer(void)
{
  if(_pu1TxPtr != NULL)
  {
    ...
    {
			/* Write one byte to the transmit data register */
			USART_SendData(EVAL_COM5, _pu1TxPtr->pscommTxContent[_u1txCnt]);
			_u1txCnt++;
...
//    }     
  ...
  if(_u1RcvStates == URX_BUF_COMPL)
  {
    ...
    _u1RcvStates = URX_BUF_READY;
		OSMboxPost(Rs232_mbox,(void *)_pu1RxPtr); 	//ucos application: send msg to mail box use
//    _u1TxEn = ENABLE;
//    if(_u1Mode == SCOMM_MODE_MASTER)
//    {
//      _u1FrameTmr = 0;
...
scommReturn_t  t_hwuart5_ReceiveMessage(scommRcvBuf_t *rxMsg)
{
...
scommReturn_t  t_hwuart5_SendMessage(scommTxBuf_t*  txMsg)
{
//  if(_u1TxEn == ENABLE)
//  {
//    if(_u1Mode == SCOMM_MODE_MASTER)
...
//    }
    ...
    if((txMsg != NULL) && (txMsg->scommTxLen > 0))
    {
      ...
      _u1txCnt = 0;//1;   
//      _u1WrTxRegEn = TRUE;    
			USART_ITConfig(EVAL_COM5, USART_IT_TXE, ENABLE); //add yan141111
			_u1Txing = TRUE;

//			/* Write first byte to the transmit data register */
//			USART_SendData(EVAL_COM5, _pu1TxPtr->pscommTxContent[0]);//[_u1txCnt-1]);
      return(SCOMM_RET_OK);
    }
    else
    {
...
u8  u1_hwuart5_txing(void)
{
	return ((USART_GetITStatus(EVAL_COM5, USART_IT_TXE) == SET) ||(_u1Txing == TRUE));
}
...
void os_reset_rcvstate_u5(void)			//mail box use
{
	_pu1RxPtr->scommRcvLen = 0;   //reset value
	_u1RcvStates = URX_BUF_IDLE; 
}	
u8 os_get_uart_chls_u5(void)
{
