...
__align(4) ETH_DMADESCTypeDef *DMARxDscrTab = NULL;	//以太网DMA接收描述符数据结构体指针
__align(4) ETH_DMADESCTypeDef *DMATxDscrTab = NULL;	//以太网DMA发送描述符数据结构体指针 
__align(4) uint8_t *Rx_Buff = NULL; 					//以太网底层驱动接收buffers指针 
__align(4) uint8_t *Tx_Buff = NULL; 					//以太网底层驱动发送buffers指针
...
{
	u8 link;
	link=((ETH_ReadPHYRegister(0x00,29)&0x10)>>4); //从LAN8720的29号寄存器读取连接状态中断:1为断开
	return link;
}
//读取网线连接状态
//0-link down, 1-link up
u8 LAN8720_Get_basic_link(void)
{
	u8 link;
	link=((ETH_ReadPHYRegister(0x00,1)&0x04)>>2); //从LAN8720的1号寄存器读取link status
	return link;
}
/////////////////////////////////////////////////////////////////////////////////////////////////
