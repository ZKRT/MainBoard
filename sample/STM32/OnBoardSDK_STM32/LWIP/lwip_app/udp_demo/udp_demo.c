...
void udp_demo_set_remoteip(void)
{
	...
	printf("Remote IP:%d.%d.%d.",lwipdev.remoteip[0],lwipdev.remoteip[1],lwipdev.remoteip[2]);//远端IP
} 
...
void udp_demo_test(void)
{
	...
	printf("Remote Port:%d",UDP_DEMO_PORT);//客户端端口号
	udppcb=udp_new();
	if(udppcb)//创建成功
	{ 
		IP4_ADDR(&rmtipaddr,lwipdev.remoteip[0],lwipdev.remoteip[1],lwipdev.remoteip[2],lwipdev.remoteip[3]);
		err=udp_connect(udppcb,&rmtipaddr,15020);//UDP客户端连接到指定IP地址和端口号的服务器
		if(err==ERR_OK)
		{
