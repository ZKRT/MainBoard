...
typedef struct __mavlink_message 
{
	...
	uint64_t payload64[(MAVLINK_MAX_PAYLOAD_LEN+MAVLINK_NUM_CHECKSUM_BYTES+7)/8];//(255+2+7)/8，也就是保存了(数据位+校验位+7)以形成8的倍数，可以直接除以8。
} mavlink_message_t;                                                           //这里为什么定义成uint_64类型的而不是uint_8型？
                                                                               //我猜测，可能是定义的时候尽量最大化，以防止有64位数据传进来会造成数据丢失
                                                                               //如果正常传输8位字节的，那么在数据地址将数据转化为8位就可以了。详情见接收字符函数
...
/*
定义msg里的数组payload是64位类型的数组
当接收8位数据时，取一个index标志位作为接收到的个数与数组位置
每次接收时，指定payload[0]的地址，再将这个地址墙砖为char*型，再在这个地址基础上偏移index个位置，再取这个地址的数据，将这个地址的数据赋值进去，完成操作
表达式1：*((char*)(&payload[0])+index) = value;
表达式2：((char*)(&payload[0]))[index] = value;
