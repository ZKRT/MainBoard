						       ...
						       uint8_t type, uint8_t autopilot, uint8_t base_mode, uint32_t custom_mode, uint8_t system_status)
{
  ...
  memcpy(_MAV_PAYLOAD_NON_CONST(msg), &packet, MAVLINK_MSG_ID_HEARTBEAT_LEN);
	...
	msg->msgid = MAVLINK_MSG_ID_HEARTBEAT;//msgid=0
...
#endif  //对应MAVLINK_MSG_ID_HEARTBEAT_LEN <= MAVLINK_MAX_PAYLOAD_LEN

#endif  //对应MAVLINK_USE_CONVENIENCE_FUNCTIONS
...
#else
	memcpy(heartbeat, _MAV_PAYLOAD(msg), MAVLINK_MSG_ID_HEARTBEAT_LEN);//以payload为基地址，将相应长度（这里心跳包长度是9）的数据保存到heartbeat结构体里
#endif
}
