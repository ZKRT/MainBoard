...
static char getchinfo_gasptf(void *sdata, void *rdata, u8 slen, u8* rlen)
{
	...
	if(res==S_Success_Gas)
	{
		if(sother->ch <BSA_GAS_NUM)
		{
			...
			rother->sensor_type = manu_gastype2zkrt(GASTYPE_MAP_BSA_INDEX, bsa_chgas_info[sother->ch].sensor_type);
			rother->unit_type = manu_gasunittype2zkrt(GASTYPE_MAP_BSA_INDEX, bsa_chgas_info[sother->ch].unit_type);
