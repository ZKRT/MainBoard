...
uint8_t *DjiDataformMobie=NULL;
uint8_t msg_buffer[25]={0};
uint8_t i=0;
extern uint8_t djidataformmobile[100];
...
void DJI::onboardSDK::CoreAPI::recvReqData(Header *protocolHeader)
{
  ...
  if (getCmdSet(protocolHeader) == SET_BROADCAST)
  {
    ...
    DjiDataformMobie = ((unsigned char *)protocolHeader + sizeof(Header) +2);	
		  for(i=0;i<50;i++)
	 {
	//	printf("0x %d   ",*DjiDataformMobie);
		djidataformmobile[i]=*DjiDataformMobie;
		DjiDataformMobie++;
	  }
     if (fromMobileCallback.callback)
       {
        printf( "Receive data from mobilen\r\n");
        fromMobileCallback.callback(this, protocolHeader, fromMobileCallback.userData);
        }
     else
    {
   printf( "parseFromMobileCallback\r\n");
